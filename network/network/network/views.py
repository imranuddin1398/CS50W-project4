...
def remove_like(request, post_id):
    ...
    like.delete()
    like_count = Like.objects.filter(post=post).count()  # Count the likes for the post
    return JsonResponse({"message": "Like removed!", "new_like_count": like_count})
...
def add_like(request, post_id):
    ...
    newLike.save()
    like_count = Like.objects.filter(post=post).count()  # Count the likes for the post
    return JsonResponse({"message": "Like added!", "new_like_count": like_count})
...
def index(request):
    ...
    whoYouLiked = [like.post.id for like in allLikes if like.user.id == request.user.id]
